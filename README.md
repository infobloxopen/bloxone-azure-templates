...
# Secret
Key Vault Secret Name: SecretName
```
Will be passing cloud init data, pass the below one while testing with ARM Templates

```bash
#cloud-config
write_files:
- path: /etc/infoblox/host_setup.yaml
  permissions: 0644
  owner: root:root
  content: |
    host_setup:
      jointoken: PLACEHOLDER
      tags:
        - DeployedFrom: AZURE-Marketplace
runcmd:
  - bash -lc 'ACCESS_TOKEN=$(curl -s -H Metadata:true "http://169.254.169.254/metadata/identity/oauth2/token?api-version=2018-02-01&resource=https://vault.azure.net" | python3 -c "import sys,json;print(json.load(sys.stdin)[\"access_token\"])")'
  - bash -lc 'JOINTOKEN=$(curl -sS -H "Authorization: Bearer ${ACCESS_TOKEN}" "https://REPLACE_VAULT_NAME.vault.azure.net/secrets/REPLACE_SECRET_NAME?api-version=7.4" | python3 -c "import sys,json;print(json.load(sys.stdin)[\"value\"])")'
  - bash -lc 'sed -i s#PLACEHOLDER#${JOINTOKEN}#g /etc/infoblox/host_setup.yaml'
  - bash -lc 'if [ "REPLACE_HTTP_PROXY" != "" ]; then echo "export http_proxy=REPLACE_HTTP_PROXY" > /etc/profile.d/infoblox-proxy.sh; echo "export https_proxy=REPLACE_HTTP_PROXY" >> /etc/profile.d/infoblox-proxy.sh; fi'
```
